...
export default {
  config: {
    sessionLength: {
      ...
      description: 'For each session, how much time do you want to work for before a break?',
      type: 'integer',
      default: 25,
      minimum: 5
    },
    beep: {
      title: 'Beep On Action',
      description: 'Beep on start, stop, pause, etc.',
      type: 'boolean',
      default: true
    }
  },
  ...
  showStats () {
    ...
    this.notify(title, `${today} ( ${history} )`)
  },
  ...
  beep () {
    if (atom.config.get('tomatimer.beep')) atom.beep()
  }
}
